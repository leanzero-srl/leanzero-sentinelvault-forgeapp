...
| A7 | **Cross-space "My work" page** (approvals waiting on me + my seals + my requests) | Comala: Document Report filters "My assigned/pending approvals"; Lockpoint: none | `list-my-approvals`, `enumerate-operator-seals`, `list-my-edit-requests` all exist; add a `confluence:globalPage` that composes them. | New manifest module (no scope) → major bump for re-consent? verify: modules without scopes are minor on Forge |
