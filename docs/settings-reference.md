...
| Show the banner from | `ribbonThresholdLevel` | Level id or null | null (= the scheme's highest level) | CLS-10 (2026-09-20): in "Exceptions only", a page classified at this level or higher opens the ribbon on its own. A level PICKER (chips), never a rank number; the rank is resolved against the scheme at read time, so renaming or re-ranking a level keeps the threshold on the same level. `ribbonThresholdRank` (1–99, default 4) stays as the stored fallback for the config API and installs older than CLS-10 — it has no row in the console. Hidden while classification is off. |
