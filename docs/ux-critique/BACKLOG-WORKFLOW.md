...
**Status (2026-09-20):** done in the WF-10 + WF-11 commit (dev 8.27.0), evidence `scenarios/sentinel-vault/wf10-wf11-workflow-tab.spec.ts` (browser: the Workflow tab opens with ONE rule sentence — `Pages start in Draft. Mihai Perdum approves before a page is Approved. Approved pages are protected: an edit by anyone who is not an approver or a space admin moves the page back to Draft. Re-review after 150 days.` — exactly one Save button (`Save workflow settings`) and no definition editor on that view; `Edit the states…` beside the state chips switches the tab to the states view, where the settings Save is gone, each workflow has its own `Save workflow`, and `← Back to workflow settings` returns; the two-buttons explainer sentence is gone). Shipped: `kit/workflow-rule.js workflowRuleSentence(settings, def)` (pure, `test/workflow-rule.test.mjs`), `WorkflowSettingsEditor` prop `onEditStates`, `WorkflowDefinitionEditor` props `standalone` / `onBack`, `realm-console` `wfView`. **Done (2026-09-20, dev 8.36.0):** the approval and protection settings moved INTO the states view under the default workflow (`WorkflowApprovalSection` / `WorkflowProtectionSection`, exported from WorkflowSettingsEditor and rendered by the standalone WorkflowDefinitionEditor), with ONE `Save workflow` that calls `save-workflow-bundle`: the definition is written first, then the settings; a refused settings write puts the definition back exactly as it was (`restoreSpaceDefaultWorkflow`) and the editor says "Nothing was saved — the settings were refused (…); the states were put back as they were" (`workflow/bundle.js runBundle` + `bundleFailureText`, pure, `test/workflow-bundle.test.mjs`). The settings view keeps the workflow switches, readers and the rule sentence; its button reads "Edit the states, approvers and protection…". Readers stay on the settings view (they are not part of the definition). Evidence `scenarios/sentinel-vault/wf11-bundle-save.spec.ts` (server: a bundle with `demoteTo: no_such_state` answers `stage: settings, restored: true` and the space's definition record is byte-identical to before; a good bundle writes both; a non-steward is refused; browser: the settings view has no Approval/Protection section and one Save, the states view has both under the default workflow with one Save, a renamed state + a 99-day clock save together — "Workflow and its settings saved.", light + dark PNGs read) + `wf10-wf11-workflow-tab.spec.ts` + `realm-workflow-definition.spec.ts` updated for the moved sections.
