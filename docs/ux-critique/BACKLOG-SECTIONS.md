...
**Status (2026-09-20):** done in the SEC-2 commit (dev 8.18.0; 8.19.0 shortens the ribbon sentence and adds the badge suffix), evidence `scenarios/sentinel-vault/sec2-workflow-owns-seals.spec.ts` (server: approving the page puts `workflowHeld{remainingMs…}` on Gabriela's section seal and pauses its expiry; her release / extend / grant and Mihai's request are refused with "Locked by the approval of this page — changes go through the workflow"; Mihai — approver, not the owner — edits inside the section over REST and is KEPT: the snapshot re-baselines, no restore write (v2 → v3), the page stays Approved with the baseline at v3; moving to Draft hands the seal back with the time it had left and the owner can release again; browser: the ribbon's Locked pill says "locked by the approval of this page" with no Request edit, the modal row's primary is the held state with only Copy link / Force release under ⋯, light + dark). Shipped: (a) custody + one privileged set, (b) approver edits re-baseline, (c) hand-back with remaining time, (d) the approval dialog lists what it will freeze, plus "Locked by the approval of this page" on the rows, the ribbon, the macro badge and My work. **Attachment seals verified live (2026-09-20):** `sec2-workflow-owns-seals.spec.ts` "SEC-2 server (attachments)" — Gabriela's file seal is listed by `workflow-seals-to-freeze`, held on Approved (`workflowHeld`, `expiresAt: null`, the space index row follows, the modal row says so), her `unseal-artifact` / `extend-seal` / `grant-edit-access` and Mihai's `request-edit-access` answer the one sentence, Draft hands it back with the remaining time and she releases it. **Held refusals for a NON-STEWARD owner verified live (2026-09-20, plain-editor bed):** `plain-editor-refusals.spec.ts` — PLAIN seals Decisions himself, the page is Approved, his release / extend / grant and his request on Gabriela's held seal all answer the one sentence; handed back, he releases his own. **(e) done (2026-09-20, dev 8.37.0):** the held row's primary is `Propose a change` (row-state `propose`, for owner and stranger alike); it opens the existing edit-request bar prefilled "Proposed change to an approved page" (panel + modal, button `Propose`), and the request lands as a PROPOSAL addressed to the page's APPROVERS (`proposal: true, approvers: […]` on the record, one owner-index row per approver so it sits in the approvers' My work, the mail goes to them) — never to the seal's owner. An approver's Approve is the workflow's own door: the page is moved back to the demote target (custody hands every seal back), then the ordinary grant is minted; Decline is the usual decline with a word. Sections and attachments alike. Evidence `scenarios/sentinel-vault/sec2e-propose-change.spec.ts` (server on the plain-editor bed: PLAIN's proposal names Mihai, Mihai's inbox lists it and Gabriela's does not, PLAIN cannot approve his own, Mihai's Approve → page Draft, section + file handed back, grant minted, `check-section-edit` granted; a held-file proposal declined by Mihai with "not while the audit runs" moves nothing; browser: the modal row reads Propose a change with the sentence "Locked by the approval of this page · expiry paused", the bar comes prefilled, after Propose the row reads `Waiting for the approvers`, light + dark PNGs read). Unit: `test/page-details.test.mjs` (held rows), `test/my-work-index.test.mjs` (proposal rows). (f) skipping the section pass in revert mode (it already stays off while the page revert runs — `ctx.enforcedRevert`; in demote mode the page is no longer enforced after the demote and the section pass runs on the handed-back seal, so "Restored" + "Moved back" can still both show on that one save).
...
**Status (2026-09-20):** done in the SEC-3 commit (dev 8.21.0), evidence `scenarios/sentinel-vault/sec3-status-language.spec.ts` (server: the byline title reads `Unclassified · Draft · Sealed (2)` / `Draft · Sealed (2)` / `Approved vN · Sealed (2)`, a release refusal never says "unseal"; browser: the modal row, the panel row and the macro badge carry the SAME sentence — `Sealed by you · until Tue 02:07` / `Locked by Gabriela Perdum · until Tue 02:07` — and the ribbon's clock is the same instant in the same format; the editor's pill reads `Undone`; on an Approved page the word "Approved" appears once in the bar (`Approved v4 ▾` on the state chip, `review due Feb 16 ▾` on the details chip); light + dark PNGs read). Shipped: `src/ui/kit/status-language.js` is the ONE vocabulary + ONE clock (`when`: weekday + 24-h inside 7 days, `22 Sep 22:57` beyond, year when it differs) — `ribbon-rules.untilLabel`, `seal-row.when`, the modal's `when`, the panel's `renderLapseDate`, the macro's `fmtUntil` and My work's `when` are all that function now; `sealSentence` / `stateText` / `attachmentChip` / `alertWord` / `sealCountLabel` feed the surfaces; verbs: "unlock" (My work), "unseal"/"Unseal"/"Unsealed" (space console, overlay watch column, flash, refusals, ribbon) → Release/release; "Overdue"/"My Seal" chips → Expired / Sealed by you; the byline title carries `Sealed (N)` (the source moves to the tooltip when a status or a count is on the chip). Unit: `test/status-language.test.mjs`. NOT changed on purpose: the ribbon's Locked sentence keeps "is sealed by … until" (the pill carries the state word), and the notice/ribbon sentences keep the verb "reverted" (the state word is Undone).
...
**Status (2026-09-20):** (b) done in the CLS-10 / SEC-4 commit (dev 8.29.0), evidence `scenarios/sentinel-vault/sec4-seal-section-doors.spec.ts` (browser: a page with NO Sentinel Vault macro — the byline's details modal reads "Nothing on this page is sealed. Seal a section here, or seal attachments from the Attachments tab." with a `Seal a section…` button; the picker lists the headings with the range preview, a click opens the Holds-for step (space default preselected) with a note, `Seal this section` seals for the chosen day, the row appears as `Sealed by you · until …` with the note, and the page's macro badge shows the seal — three clicks from the byline, no page edit). The macro's badge / dialog / lock hint / undone hint now name Sentinel Vault under the page title as a door as well as the panel; the panel group note reads "Freeze a heading and everything under it; only you and the people you approve can change it" (SEC-3 commit). **(c) PARKED:** a second `confluence:contentAction` ("Seal a section…" in the page ⋯ menu) is a new manifest MODULE — outside the group's guardrail (no new modules or scopes; a module change is a major + admin re-consent on every site). **(a) done (2026-09-20, dev 8.34.0 → 8.35.0):** SEAL ON INSERT. The macro's Insert leaves a 48 h marker (`section-insert-intent` → `section-insert-pending-{pageId}`); the publish's page-content trigger — or the view-time guard, whichever judges the version first (one claim per page+version, `section-adopt-claim-…`, so the two never both mint a record) — finds the wrapper with no record (`doc-surgery findUnrecordedWrappers`, pure) and adopts it for the person who published: record + snapshot + space index row + activity `section.sealed {adoptedOnInsert}` (`section-seals/adopt.js`), the app-issued id stamped onto the node (`stampSectionId`) in the same write; the marker is consumed. Timing: inserted but never published → no event, no record, the marker expires; a page with no marker and no seal is never read (the cheap probes stay). The macro's config copy says publishing seals it; the view badge says "Sealed to you on publish — reload" when the guard reports `adoptedOnPublish`. Evidence `scenarios/sentinel-vault/sec4-seal-on-insert.spec.ts` (the REAL page event adopts: v3 by the app, one record for the author, the wrapper stamped, the marker gone; a no-marker wrapper is untouched; the guard adopts once the marker is there) + `section-macro-render.spec.ts` (the badge text); unit `test/doc-surgery.test.mjs`. Limit: the harness holds one token, so the publisher in the spec is the harness user. (d) done: the panel stays as a mirror.
...
**Status (2026-09-20):** done in the SEC-7 commit (dev 8.17.0), evidence `scenarios/sentinel-vault/sec7-section-extend.spec.ts` (server: seal-section takes `lockDuration` + `note` and the rows show them; `extend-section` exists — anchored on the live expiry, from now when lapsed, the grant carried forward, the space index row and the trail updated, no re-baseline; the lapsed refusal now names `⋯ → Extend the seal` and Extend re-arms the grant; browser: the owner's modal row says "expires tomorrow" in amber (rgb 180,83,9) with Extend under ⋯ and the warning clears after the extension; the panel's picker opens a "Holds for" step with the space default preselected, a note, and seals for the chosen day). (a)(b)(c) and the amber half of (d) shipped; REST ops `extend-section` / `extend-attachment`. **Non-owner non-steward refusals verified live (2026-09-20, plain-editor bed):** `scenarios/sentinel-vault/plain-editor-refusals.spec.ts` ("SEC-7 + SEC-2") — PLAIN (a real editor, not a steward, in SVPLAIN) is refused `extend-section` (`Only the seal owner or a space admin can extend this seal`), `unseal-section` (`Only the section owner or a space admin can release this seal`) and `grant-section-edit` (`Not the section owner`) on Gabriela's seal, the record unchanged; Mihai — site admin, not the owner — extends through the same seam. **(d) second half done (2026-09-20, dev 8.31.0):** the expiry sweep walks `section-protection-` beside `protection-` through ONE pure mapping (`shared/lapse-policy.js lapseSubject` — a workflow-held seal, expiry paused, is skipped), posts the same halfway / overdue (1 of 3) / auto-release notices with the word "section" (`targetKind` on the four expiry layouts and mailers), records `reservation-expired` / `seal-auto-released` dispatches carrying `sectionId`, and hands the section back through the ONE teardown `section-seals/release.js releaseSectionSeal` (wrapper unwrapped, records + index + counters gone, activity `section.auto-released`) that `unseal-section` now uses too. Evidence `scenarios/sentinel-vault/sec7-section-sweep.spec.ts` (server: the three sweeps, the comments on the page, the counter on the section's key, the body back on the page with its text) + `expiry-sweep.spec.ts` unchanged for attachments; unit `test/lapse-policy.test.mjs` (lapseSubject) + `test/notice-kinds.test.mjs` (the four layouts).
...
**Status (2026-09-20):** done in the SEC-8 commit (dev 8.23.0), evidence `scenarios/sentinel-vault/sec8-declined-state.spec.ts` (server: `deny-section-edit` / `deny-edit-request` take an optional `reason` stored as `deniedReason`; `check-section-edit`, `page-details-summary` rows (`myDeniedReason`) and `ribbon-summary.lockedFor` (`myRequest: "denied"`, `retryAt`, `deniedReason`) carry it; asking again is refused with `retryAt` and a sentence with no UTC/ISO stamp; `list-my-requests` answers the requester's own pending / declined / granted rows with names and pages; browser: the ribbon pill reads `Declined · ask again Sun 03:16` with the owner's word in the sentence and no Request edit; the modal row's primary is the `declined` state span with the word on the row; the owner's Decline opens an optional-reason bar and the reason lands on the record; My work's new "Your edit requests" card lists it as Declined with the word and the clock; light + dark PNGs read). Shipped: row-state `denied` → `{ kind: "declined", retryAt, reason }` until `retryAt` passes, then Request edit again; `pickUrgent` kind `declined`; `DECLINED_REASON` + `refusalText` (status-language) — the server never formats a clock; requester index `editreq-mine-{me}-{kind}-{id}` (written on request and on direct grant, healed on read); REST `decline-attachment-edit` / `decline-section-edit` with `reason`. NOT done: "Try again in 1 h" as a button (the state names the time; the row becomes Request edit by itself when it passes) and "Ask Gabriela on the page" (a comment @mention) — the ribbon/row sentence names the owner and the direct-grant path exists.
