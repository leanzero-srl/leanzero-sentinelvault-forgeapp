...
const ArtifactCard = ({ att, onRefresh, columns, siteUrl, spaceKey, pageId, pageLocation, viewer, editInfo, onMoved, justMoved = false }) => {
...
const ArtifactGridView = () => {
  ...
  const renderCards = (files) =>
    files.map((att) => (
      <ArtifactCard key={att.id} att={att} columns={cols} onRefresh={onRefresh} onMoved={onMoved} justMoved={!!recent[att.id]} siteUrl={siteUrl} spaceKey={spaceKey} pageId={pageId} pageLocation={pageLocation} viewer={viewer} editInfo={editStatusById[att.id]} />
    ));
