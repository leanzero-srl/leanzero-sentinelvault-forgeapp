...
- LEAD (it31, terminology micro-inconsistency — candidate for a future microcopy tick, NOT chased now to avoid over-indexing on microcopy): the SAME concept "the current user owns this seal" is labelled **"MY RESERVATION"** on the inline-panel card badge but **"YOURS"** on the overlay (Manage Attachments modal) card badge. Converge on one term across the two surfaces. Verify both live before changing (grep the badge strings; keep the seal/reservation metaphor consistent with the rest of the app copy).
