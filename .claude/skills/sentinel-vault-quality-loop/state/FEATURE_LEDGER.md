...
- **A11y coverage (loop angle):** menu roving-focus (it2), Toggle/status-banner labels (it4), approval `role="dialog"` focus-on-open + Escape focus-return (it14, scripted-verified). OPEN a11y leads: WorkflowInbox Approve/Deny per-item accessible names; WorkflowDashboard `<th scope="col">`; the mouse-only dropdowns above still need the shared keyboard `Menu` primitive.
