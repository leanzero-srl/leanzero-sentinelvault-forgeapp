...
- **Failure-surfacing (loop angle):** all four settings saves (`set-space-workflow-settings`, `store-validation-config`, `store-policy`×2) now check the resolver's `{success:false,reason}` return instead of showing a false "saved" (it16). NOTE: `invoke()` resolves for a business `{success:false}` — only a return-check surfaces it. **OPEN AUTHZ LEAD:** `store-validation-config` (`storeConfig`) has NO server-side authz gate (unlike `store-policy` post-A1) — gate it in a future authz tick.
