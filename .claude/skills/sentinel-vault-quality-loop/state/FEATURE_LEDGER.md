...
- **Copy/microcopy (loop angle):** reviewed it19 — copy is generally strong. **OWNER DECISION FLAGGED:** realm-console uses themed "realm" ×14 (title/tab/subtitle) while the newer editors + invitations use Confluence-native "space" ×4 — same screen, same concept. "realm" uniquely collides with Confluence's "space" (steward/guild/seal are app-unique, fine). Recommend standardizing user-facing scope term on "space"; NOT changed unilaterally (brand call). Secondary: "guild" vs "group".
