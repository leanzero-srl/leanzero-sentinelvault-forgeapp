...
- **A11y coverage (loop angle):** menu roving-focus (it2), Toggle/status-banner labels (it4), approval `role="dialog"` focus-on-open + Escape focus-return (it14, scripted-verified). OPEN a11y leads: WorkflowInbox per-item accessible names DONE (it21); WorkflowDashboard `<th scope="col">` still open; the mouse-only dropdowns above still need the shared keyboard `Menu` primitive.
