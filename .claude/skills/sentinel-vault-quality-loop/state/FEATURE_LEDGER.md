...
| 49 | Document Activity per page + space Activity report + CSV (Comala parity A1) | Inline panel + overlay + realm console tab | **implemented (it62) — live-verified on wolfaenpak: activity-log.spec 3/3, activity-surfaces.spec 2/2; red-team review 9 findings, 8 fixed, 1 accepted (page readers see request reasons — same audience as the comment already posted on the page)** | 5 | 4 | 3 | 4 | 4 | 22.0 | `infra/activity-log.js` (two keys per event, newest-first inverted ts, no TTL), 31 write sites, `activity` capsule (page feed gated on read, space report on stewardship, titles/names resolved by the reader), kit ActivityFeed/ActivityReport, mock + live PNGs |
| 50 | Approval evidence: per-decision record kept after completion, denial + stale traces (A4) | Ribbon evidence chip + dialog; state record + workflow log | **implemented (it63) — live-verified: approval-evidence.spec 2/2, ribbon-approval-dialog extended; review 9 findings, 9 fixed** | 5 | 4 | 3 | 4 | 4 | 20.0 | `buildApprovalRecord` (pure, bounded 50 rows / 300-char reasons / 120-char names, decidedVersion), snapshot BEFORE clearPageApprovals on approve/deny/stale; cleared on leaving the enforce state; chip labelled "Approval record · v{reviewed}", baseline-moved line, CL-5-aware stale line |
| 51 | "View approved version" link on the record + the pinned version on the awaiting dialog (A6) | Doc ribbon | **implemented (it63) — live-verified by CLICK: router.navigate from the sandboxed iframe lands the top window on viewpage.action?pageId&pageVersion** | 5 | 4 | 3 | 5 | 4 | 18.0 | a plain target=_top anchor is dropped by the Custom UI sandbox (review HIGH) — navigation goes through @forge/bridge router; classic viewpageversion.action is a 404 on Cloud |
| 52 | Configurable on-tamper target state (demote-to In Review) + editor/version in demote comment (A2) | Backend enforcement + settings editor | **implemented (it64) — `workflow-settings.demoteTo` validated at save (exists, non-enforce, edge from every enforce state, no entry condition); both demote sites honour it; DEFAULT_WORKFLOW gained approved → in_review; live: workflow-demote-target.spec 2/2** | 4 | 4 | 3 | 5 | 4 | 19.0 | Comala "updated → Review" parity; a demote skips the entry gate so a gated state cannot be the target |
| 53 | Per-state expiry + steward-editable review date in the dialog (A5) | Backend sweep + ribbon dialog + settings | **implemented (it64) — per-state clocks, `set-review-due` (edit + steward, past refused), custom DatePicker in an in-flow dialog, sweep expires ANY overdue state with an edge to Expired and announces once (marker, no TTL) otherwise; UTC end-of-day dates rendered in UTC everywhere; live: review-due 2/2, ribbon-review-due 1/1 (dialog inside the iframe box)** | 4 | 4 | 3 | 5 | 4 | 18.0 | Comala due-date parity; the marker's 7-day TTL would have re-announced pages without an Expired edge weekly forever (review) |
| 56 | Trashed / purged pages leave the workflow readers (dashboard, inbox, sweep) | Backend + dashboard | **implemented (it64) — `shared/page-status.js` batch lookup (250 ids/call); trashed → hidden + skipped, record kept; missing (404) → keys purged by the sweep; live: workflow-trashed-pages.spec** | 5 | 4 | 4 | 5 | 5 | — | found by the harness: all 13 rows on the WFH dashboard were trashed harness pages; a negative that licenses a purge needs BOTH lookups to have succeeded |
| 57 | Label sync — the workflow state as a page label `sv-state-{id}` (B4) | Backend + settings | **implemented (it67) — `workflow/label-sync.js`, setting `syncLabels`, sweep backfill/removal; live: workflow-label-sync.spec** | 4 | 4 | 4 | 5 | 4 | — | content-property CQL does not parse on Forge; no new scope (v1 label API under write:confluence-content) |
| 58 | Read confirmations — "I have read version N" on Approved pages (B2) | Backend + ribbon + settings | **implemented (it67) — `read-ack-*`, confirm-read / get-read-status / get-read-report (names steward-only), per-version acks; live: read-confirmations + ribbon-read-confirm** | 4 | 4 | 4 | 5 | 4 | — | Comala sells this as a separate app |
| 59 | Signed approval decisions — TOTP e-signature (B3) | Backend + ribbon + My work + settings | **implemented (it67) — `shared/totp.js` (RFC vectors), `workflow/signature.js` (enrol/confirm/replay/revoke), `requireSignature`, decisions carry `signed`; live: esignature + ribbon-esignature (caught a memoised empty code)** | 4 | 4 | 4 | 5 | 4 | — | not a Part 11 claim; Forge has no re-auth API so this is the honest maximum |
| 60 | Definition editor + label-scoped workflows with priority (B1) | Backend + console editor | **implemented (it67) — validateDefinition, chooseWorkflowForLabels, `workflow-def-space-{key}-{id}`, list/store/delete-space-workflow, every page-acting caller resolves by the page's workflowId; live: workflow-definitions (+ realm-workflow-definition browser)** | 4 | 4 | 4 | 5 | 4 | — | MEDIUM confidence: the per-page definition resolution touched 9 call sites; the browser editor spec waits on the shared profile |
| 61 | Outbound webhook on state change (B5) | Manifest egress | **not built — owner decision**: needs an egress permission → major version, re-consent on every install, and the end of "Runs on Atlassian" | — | — | — | — | — | — | outside "no new scope"; ask the owner explicitly |
| 62 | UX pass to production polish — one details chip on the banner, sectioned Workflow tab, dialogs reviewed light+dark | Doc ribbon + realm console + My work | **implemented (it68)** — banner: state chip + ONE outlined details chip ("Approved v1 · review due Feb 3 · read 0/2") opening a popover with Approval record / Review date / Readers sections; Workflow tab in four sections (Workflow, Readers, Approval, Protecting Approved pages); definitions editor grid + save-scope note; sign-to-move dialog shows a refused code as an in-dialog error, neutral Cancel, focus back to the code field; pickers get a full-width search box; dark-mode empty inbox border fixed; dead chip CSS removed. Screenshots read in both themes (20 shots). | 4 | 4 | 3 | 4 | 4 | — | main |
| 63 | Violation-comment dedup races closed (SECURITY-TODO 'Known, pre-existing' 1 + 2) | triggers.js + shared/notice-dedup.js | **verified (it69)** — race 1: a clean save re-arms only when a USER authored the version read (`decideClear`, v2 `version.authorId`); race 2: per-outcome claim token, write → settle 400ms → re-read → announce only if held (`confirmClaim`), clobbered sibling outcomes merged back (F2 kept). 60 unit assertions; live 12/12 race-spec runs (was ~1 in 3 failing), REST lanes 2–6 84/84; the kept-markers log line fired 3× in 4 runs. | 5 | — | — | 4 | — | — | main |
| 54 | Cross-space "My work" global page (approvals + seals + requests) (A7) | New confluence:globalPage | **implemented (it65) — `confluence:globalPage` my-work composing list-my-approvals / list-my-edit-requests / enumerate-operator-seals (all caller-scoped); Approve/Deny from the page; links via router.navigate; the module was a MINOR bump (6.97 → 6.100, install stayed Up-to-date); live: my-work-page.spec 1/1** | 4 | 4 | 4 | 5 | 4 | 16.0 | a scope-free module IS a minor bump (proven on dev); open: list-my-edit-requests is a site-wide scan filtered client-side (K1 index discipline) |
| 55 | Per-state page restrictions as optional add-on to enforcement (A3) | Backend + settings | researched | — | — | — | — | — | 14.0 | Comala add/set/remove-restrictions parity; LOW confidence until the v1 restrictions API is probed under Forge; likely new scope → major |
...
- **STATUS 2026-07-05 (iteration 3):** AT-SCALE ASSIGNMENT (auto-assign-on-create) VERIFIED (`iterations/03-workflow-autoassign.md`). Per-space `workflow-settings-{key}` (enabled/autoAssignNew) + a create-only auto-assign pass in `pageContentTrigger` → new pages in an enabled space get a workflow automatically (workflow-autoassign-e2e 8/8). 3-lens adversarial trigger-safety review: loop-safety clean; fixed a dedup-marker concurrency gap (T6 pattern) + a create-only gate.
- **STATUS 2026-07-05 (iteration 4): #42 COMPLETE / VERIFIED** (`iterations/04-workflow-config-ui.md`). Steward realm-console "Workflow" tab (`WorkflowSettingsEditor` kit mirroring `ValidationsEditor` — matches the app in light+dark) with enable/auto-assign toggles, read-only state preview, and bulk "Apply to existing pages" (`bulk-assign-workflow`, bounded ≤25/call, cursor-resumed). workflow-bulk-e2e 6/6 (isolated throwaway space). Adversarial review fixed a HIGH bulk-cursor bug (kit dropped `nextCursor` — pages 26+ unreachable; verified fixed via `bulk-cursor.mjs`) + Toggle accessible-name + banner aria-live (applied to both kits). Fn 4→5. **The workflow-rules capability (#42) is end-to-end shipped.** Next in the pack: #43 (multi-approver transitions) binds to this engine.
...
- **A11y coverage (loop angle):** menu roving-focus (it2), Toggle/status-banner labels (it4), approval `role="dialog"` focus-on-open + Escape focus-return (it14, scripted-verified). OPEN a11y leads: WorkflowInbox per-item accessible names DONE (it21); WorkflowDashboard `<th scope="col">` DONE (it22); a11y-semantics on workflow surfaces now clean; the mouse-only dropdowns above still need the shared keyboard `Menu` primitive.
- **Responsive coverage (loop angle):** realm-console + steward-console reflow to 320–360px (it15 — tab-nav wrap, removed `min-width` canvas/card pins, `.settings-row` stacks < 640px; WCAG 1.4.10; desktop unchanged). Reusable probe: viewport-loop + `scrollWidth>innerWidth`. OPEN: overlay surface `.tab-navigation` not yet narrow-audited (fixed-size modal, low risk).
- **Failure-surfacing (loop angle):** all four settings saves (`set-space-workflow-settings`, `store-validation-config`, `store-policy`×2) now check the resolver's `{success:false,reason}` return instead of showing a false "saved" (it16). NOTE: `invoke()` resolves for a business `{success:false}` — only a return-check surfaces it.
- **Copy/microcopy (loop angle):** it19 flagged realm/space + guild/group inconsistency → **it20 RESOLVED per owner directive** ("everything Confluence-native, keep sealing"). All 41 user-facing realm→space + guild→group; kept seal/steward/Sentinel Vault (app-unique). Verified app-wide, grade PASS on deployed v6.9.0. **OPEN OWNER FLAG:** "operator" (app's word for a user, e.g. "Add Operator") is also non-native — awaiting owner's preferred target (user/person/member?) before a sweep. Internal code (realmKey, `space-protection-*` keys, `scope==="realm"` enum, resolver keys) deliberately UNCHANGED — not user-facing.
- **Error/empty/loading (loop angle):** audited it18 — inline-panel triad + AI-poll (capped, error-handled) are SOLID; inbox null-on-empty is intentional restraint. FIXED: WorkflowDashboard now surfaces a load failure (`.wf-dash-error`) instead of vanishing silently. LEARNING: null-on-empty OK, null-on-ERROR is a silent failure — split them. OPEN: dashboard onboarding empty-state (needs resolver `enabled`); set-ai-finding-state optimistic no-rollback.
- **Authz coverage (loop angle):** `store-validation-config` now gated (it17 — global→site-admin, space→steward, mirrors store-policy A1). OPEN LEAD: `setAiFindingState` writes a per-finding annotation ungated (lower stakes — audit in a future authz tick). TEST-HOOK GOTCHA (it17): importing a HEAVY capsule `actions.js` (Queue/LLM/doc-surgery deps) into `test-hook.js` silently breaks the `testStateFn` bundle (deploy succeeds, runs stale code) — only import LIGHT modules there.
