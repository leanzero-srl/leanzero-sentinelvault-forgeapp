...
> **C6 RESOLVED (research → informed override):** the UI's own note ("if set, they override the global rules; leave empty to inherit") proves the override-not-merge behavior is INTENDED + documented — not a bug. Changing enforcement (merge) would silently alter every existing tenant, so instead the space Validations editor now WARNS exactly what an override drops (e.g. "replace all 3 global rules… including 1 required blocking rule"). Zero enforcement-behavior change. A hard "global block rules are a compliance floor" is flagged as an explicit owner decision.
> **D-tier: D4 (normalizeFindings no longer drops a ruleRef/suggestion-only finding — a false-pass), D7 (steward-request KVS keys sanitized — no throw on `~`-spaces) FIXED + tested.** DEFERRED with reasons: **D8** (remove unused `write:confluence-space` — the space `/permission/check` POST is on the critical steward path and can't be re-verified post-removal via the harness, so not removed blind), D1 (global-vs-realm AI budget accounting — product call), D2 (steward group pagination — needs live verify), D3/D6 (TOCTOU / feed-CAS — low blast, KVS-no-CAS), D5 (edit-grant revoke/list UI — a feature, not cleanup). The proper O(1) per-page seal INDEX (durable B4/C1 perf fix) is the next design task. Honest limitation: A1/A3/B6 authz can't be positively driven from the harness (asUser has no webtrigger context — same as #44).
