...
VIOLATION-COMMENT markers (stabilization hunt 2026-08-14): "delete" (trash handler) and "content-removal" (media pass) alias to ONE shared marker class `content-loss` — a UI delete of an embedded attachment fires both events, and per-class markers produced two comments for one delete (H2-F3). Section comments dedup under `section-removed`/`section-edited` keyed by sectionId (H1-F4). Claim ORDERING is claim-on-postable-only: the marker is claimed only when both comment toggles (`ENABLE_CONFLUENCE_BULLETINS` && `ENABLE_NATIVE_NOTIFICATIONS`) are ON, and is RELEASED when the post throws or reports failure (H1-F5/H2-F4) — a gated skip or transient post failure must never consume the 24h window (`postDedupedFootnote` in triggers.js is the one sanctioned path). **it69 (2026-09-06):** the marker also carries a per-outcome CLAIM TOKEN (`claims`) — a claim is write → settle 400ms → re-read → announce only if our token stands (`confirmClaim`; KVS has no CAS), and a clean save CLEARS markers only when a USER authored the version read (`decideClear` on v2 `version.authorId` ≠ app account) — the app's own restore is never a clean save. Pure rules in `shared/notice-dedup.js`; design in `state/DEDUP-RACES-DESIGN.md`.
Files: `src/server/triggers.js` (`runValidationPhase`, pipeline, `expirySweepTask`, `postDedupedFootnote`, `NOTICE_CLASS_ALIASES`), keys `validation-checked-*`, `expiry-notified-*`, `fifty-percent-reminder-sent-*`, `violation-noticed-*`.
...
**S3. OWNER INTENT (stabilization hunt 2026-08-14): an OWNER trashing their own sealed file RELEASES the seal — the trash handler (and the panel's own `delete-artifact`) converts the record to an inert S7 `trashedOnly` tracking record, never leaving a live seal that the next non-owner save would un-trash while blaming a bystander. Non-owner trash of a sealed attachment is auto-restored (PUT status "current", version+1) with the seal kept — via the shared `infra/attachment-status.js` restore used by BOTH the artifact trash handler and the page media pass (Fix 1, incident 2026-07-22: the page pass must never re-splice an ADF node whose attachment sits in trash — it un-trashes first; a version-less/container-less trash event is completed by a v2 probe, never silently abandoned). On restore FAILURE the seal is KEPT (attachment still recoverable from trash — audit C4) and the owner notified honestly (`revert-failed`); on permanent delete, seal state is cleaned and the owner notified that the file cannot be restored (`permanently-deleted`). Expired seals are inert on the trash and media paths (§6 parity).**
...
**S4. Owner edits always pass — including REMOVALS (stabilization hunt 2026-08-14): an owner (or active grantee) removing their own sealed embed from the page re-baselines the seal to `embedded:false` (baseline dropped), so future absence is sanctioned and never re-spliced or blamed on a later editor; an expired-but-unswept seal never blocks a re-seal by another user (SV-M8) and expired records are lazily deleted on touch.**
...
**K1. Every key pattern is load-bearing API: `protection-{id}`, `space-protection-{spaceId}-{id}`, `section-protection-{id}`, `section-snapshot-{id}`, `space-section-protection-{spaceId}-{id}`, `edit-request-*`/`edit-grant-*` (+ section variants) and their owner INDEX `editreq-owner-{ownerAccountId}-{attachmentId}-{requesterAccountId}` (2026-09-05: written with the request, dropped on approve/deny/withdraw/seal teardown, confirmed by key at read, backfilled hourly by expirySweepTask), `validation-*`, `ai-*`, `admin-settings-*`, `protections-last-modified`, `app-account-id`, notification/dedup keys, `workflow-*` (`workflow-state-`/`-idx-`/`-log-`/`-pending-`/`-approval-`/`-inbox-{approverAccountId}-{pageId}`/`-completing-`/`-autoassigned-`/`-integrity-notified-`/`-review-notified-{pageId}` (A5 overdue-notice marker: holds the due date it announced, NO TTL — a TTL re-announced pages with no edge to Expired weekly; deleted when the page expires)), `notify-request-{id}-{accountId}` (watches — NOT `notification-`, which is the dispatch feed; three teardown sweeps queried the wrong one until 2026-09-05), `workflow-def-space-{key}-{workflowId}` (B1 label-scoped extras; the space default stays at `workflow-def-space-{key}`), `read-ack-{pageId}-{accountId}` (B2, no TTL, an ack is FOR a version), `sig-secret-`/`sig-enroll-`/`sig-last-{accountId}` (B3 — the secret never leaves the app after enrolment; `sig-last` is the replay guard), `activity-page-{pageId}-{invTs}-{rand}` / `activity-space-{spaceKey}-{invTs}-{rand}` (A1 activity log, 2026-09-05: one entry written under both, no TTL, inverted timestamp so a prefix query is newest-first; each reader queries ONLY its own prefix) — prefixes may never be repurposed or overlapped ambiguously (the `section-protection-` vs `space-section-protection-`/`section-snapshot-` prefix layering is deliberate, see comment at `triggers.js:339`).**
...
PAGE-STATUS DISCIPLINE (2026-09-05): the `workflow-idx-*` (and seal index) rows OUTLIVE the page — no page-trashed event is subscribed — so every reader that turns index rows into work for a person (dashboard, inbox, sweep) asks `shared/page-status.js` where the pages are (one batch call per 250 ids). Trashed → hidden/skipped, record KEPT (a restore keeps state); missing (404 as app, BOTH lookups succeeded) → `purgePageWorkflow` from the hourly sweep only. Never purge on `unknown`.
PER-PAGE DEFINITION (2026-09-05, B1): a page record names its `workflowId`; every caller that acts on a page resolves `resolveWorkflowDef(spaceKey, record.workflowId)` — never the space default alone — or a label-scoped page is judged by states it does not have.
INDEX DISCIPLINE (2026-09-05): a per-caller listing must read a per-caller PREFIX, never a site-wide scan filtered client-side — `listMyApprovals` scanned every `workflow-approval-*` record (cap 1,500) and went blind once orphans from deleted pages accumulated; it now reads `workflow-inbox-{account}-*`, written with the approval records, deleted with them, backfilled + orphan-swept hourly by `workflowSweep` (`sweepApprovalIndex`, age-guarded so a request being opened is never eaten).
TTL DISCIPLINE (stabilization hunt 2026-08-14): every TTL'd write goes through `src/server/shared/kvs-ttl.js` (`setWithTtl`/`setUntil` → `{ ttl: { value: <seconds>, unit: "SECONDS" } }`, floor 60s, ceiling 364d). `{ expiresAt }` is NOT a @forge/kvs SetOption — it was silently ignored at ~15 call sites, making every "TTL'd" dedup/marker key permanent (hunt F3/H1-F1). Never pass `{ expiresAt }` to `kvs.set` again; `test/kvs-ttl.test.mjs` pins the shape.
Files: all capsules; `src/server/shared/kvs-ttl.js`; canonical inventory in this skill's APP_MAP.md §Data model.
