...
| **policies** | `load-policy` / `store-policy` (global+space unified; pause/resume timer-extension lives here), `enumerate-realm-rulesets` (legacy `*-ruleset` load/store/discard actions REMOVED 2026-09-05 — unused; `load-global-ruleset` was ungated and returned the steward roster) | `logic.js` duration/auto-unseal helpers (naming: "realm" = space) |
...
| **panels** | `enumerate-panel-artifacts` (enriched table), `label-artifact`/`unlabel-artifact`, `delete-artifact` (gated `allowArtifactDelete`; refuses others' seals; writes `trashedOnly` tracking record), `check-panel-status` (`inject-panel`/`extract-panel` REMOVED 2026-09-05, no callers), `store-doc-panel-prefs`, `upload-artifact` (base64 ≤4MB), `discover-panel-key` (`register-panel-key` REMOVED 2026-09-05, no callers), `resolve-artifact-preview` (data-URI ≤5MB) | — |
